...
void MainWindow::init()
{
    ...
    {
        RemoteNetwork::get().fetch(QUrl("https://github.com/usalko/sm/blob/currentrelease"), RemoteNetwork::RequestTypeCustom,
...
{
    QDesktopServices::openUrl(QUrl("https://github.com/usalko/sm/wiki"));
}
...
void MainWindow::openLinkBugReport() const
{
    ...
    QUrl url("https://github.com/usalko/sm/issues/new");
    url.setQuery(query);
...
{
    QDesktopServices::openUrl(QUrl("https://github.com/usalko"));
}
...
void MainWindow::editEncryption()
{
    ...
    if(cipherDialog.exec())
    {
        ...
        qApp->processEvents();
        if(ok)
            ok = db.executeSQL("ATTACH DATABASE '" + temporalFile.toStdString() + "' AS sm_edit_encryption KEY " + cipherSettings.getPassword() + ";",
                               false, false);
        qApp->processEvents();
        if(ok)
            ok = db.executeSQL("PRAGMA sm_edit_encryption.cipher_page_size = " + std::to_string(cipherSettings.getPageSize()), false, false);
        if(ok)
            ok = db.executeSQL("PRAGMA sm_edit_encryption.cipher_hmac_algorithm = " + cipherSettings.getHmacAlgorithm(), false, false);
        if(ok)
            ok = db.executeSQL("PRAGMA sm_edit_encryption.cipher_kdf_algorithm = " + cipherSettings.getKdfAlgorithm(), false, false);
        if(ok)
            ok = db.executeSQL("PRAGMA sm_edit_encryption.kdf_iter = " + std::to_string(cipherSettings.getKdfIterations()), false, false);
        if (ok)
            ok = db.executeSQL("PRAGMA sm_edit_encryption.cipher_plaintext_header_size = " + std::to_string(cipherSettings.getPlaintextHeaderSize()), false, false);

        // Export the current database to the new one
        qApp->processEvents();
        if(ok)
            ok = db.executeSQL("SELECT sqlcipher_export('sm_edit_encryption');", false, false);

        // Set user version of the new database
        qApp->processEvents();
        if (ok)
            ok = db.executeSQL("PRAGMA sm_edit_encryption.user_version = " + std::to_string(db.getPragma("user_version").toInt()) + ";", false, false);

        // We need to detach the database before proceeding
        qApp->processEvents();
        if (ok)
            ok = db.executeSQL("DETACH sm_edit_encryption;", false, false);
